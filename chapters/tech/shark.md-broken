...
Shark is a component of an open source,in-memory analytics, and
 ...
 addition Shark can be used to query data4 in HDFS, HBase, and Amazon
...
**Control over data partitioning => Fast, distributed JOINS:** Shark
	lets users partition tables using a specified key. In particular if
	tables will be *joined* frequently, then one can partition them
	using a common (“join”) key. Co-partitioning is a trick used by many
	MPP databases to speed up *joins* involving massive tables
	[@www-oreilly-shark].
	
**Machine-learning support:** RDD’s are distributed objects that can
	...
	(using the sql2rdd command) and apply machine-learning functions to
	them, all from within Shark. Currently machine-learning and analytic
	functions can be written in Scala and Java, with support for Python
	coming soon. Not only do users get the benefit of performing simple
	SQL queries and complex computations from within the
	`same7_framework`, Shark is 100X faster than Hadoop
	[@www-oreilly-shark].
